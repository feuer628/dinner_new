...
/**
 * Диалог подтверждения действия при помощи OTP-токена
 * Возвращает одноразовый пароль, введенный пользователем или null если пользователь отменил подтверждение
 */
@Component({
    // language=Vue
    template: `
        <dialog-form title="Подтверждение OTP-токеном" :closable="false">
            <div slot="content">
                <div>Введите одноразовый пароль</div>
...
export class OtpConfirmationDialog extends CustomDialog<undefined, string | null> {
